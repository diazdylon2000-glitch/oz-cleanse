/* app.js — Oz Companion (restored & refined)
   - Splash loader (Oz + bubble) restored
   - Paw-print checklist
   - Smart Coach: header is the button, no duplicate text into note field
   - Day selector centered, phase label under title
   - Notes & Photos tracked in Calendar badges
   - Groceries: budget, editable prices, light unit conversion
   - Weight chart with visible x-axis, crisp on iPhone
*/
...
(function () {
  ...
  window.addEventListener("load", () => {
    ...
    setTimeout(() => {
      ...
      if (note) note.classList.add("fade-out");
      setTimeout(() => {
...
function defaultDays() {
  ...
  return phases.map((ph, i) => ({
    day: i + 1, phase: ph, order: null, checks: {}, note: "", weight: null, photos: []
  }));
}

/* ------------ Plan data (you can still import/replace) ------------ */
const INITIAL_GOALS = {
...
/* Juices list here is per-day recipe, servings=4 (1 of each per cleanse day) */
const PLAN_RECIPES = [
...
/* ------------ Default price book (editable in UI) ------------ */
...
function convert(n, from, to, name = "") {
  ...
  if (from === "cup" && to === "lb") {
    const key = Object.keys(cupToLb).find(k => name.toLowerCase().includes(k));
...
const WeightChart = ({ series }) => {
  ...
  useEffect(() => {
    ...
    chartRef.current = new Chart(ctx, {
      ...
      plugins: [{
        id: "retina",
        beforeInit: (c) => { c.options.devicePixelRatio = window.devicePixelRatio || 2; }
      }]
    });
...
const COACH_RULES = [
  { id:"headache", test:ctx=>ctx.syms.has("headache"), tips:["Sip 8–12 oz water over 15 minutes.","Add a pinch of sea salt or electrolyte.","Dim screens and rest your eyes 5–10 minutes."] },
  { id:"dizziness", test:ctx=>ctx.syms.has("dizziness"), tips:["Sit or lie until steady.","Small juice or a pinch of salt if fasting.","Breathe in 4 / out 6."] },
  { id:"nausea", test:ctx=>ctx.syms.has("nausea"), tips:["Sip cool water or peppermint/ginger tea.","Step into fresh air.","Move slowly; avoid sudden changes."] },
  { id:"fatigue", test:ctx=>ctx.syms.has("fatigue"), tips:["Take a 15–20 min rest.","Hydrate or take electrolytes.","2 minutes of gentle stretching."] },
...
const Dashboard = ({ templates, days, setDays, recipes, goals }) => {
  ...
  useEffect(() => {
    if (Math.round(progress) === 100) { try { confetti && confetti({ particleCount: 100, spread: 70, origin: { y: .6 } }); } catch {} }
  }, [progress]);
  ...
  // Smart coach (no duplication into note)
    ...
    e("div", { className: "mast card" },
      e("div", { className: "mastRow" },
        e("div", { className: "mastLeft" },
          ...
          e("div", null,
            e("div", { style: { fontSize: 24, fontWeight: 800, letterSpacing: .2 } }, "Oz Companion"),
...
const App = () => {
  ...
  return e("div", {},
    ...
    // Tabs (CSS handles centering & safe-area)
